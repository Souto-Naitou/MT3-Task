...
void Draw()
{
	...
	{
		Vector3 scrpt1 = Multiply(0.5f, Add(aabb.min, aabb.max));
		Vector3 scrpt2 =  Add(Multiply(0.5f, segment.diff), segment.origin);

		ScreenPrint(scrpt1, viewProjectionMatrix, viewportMatrix, "AABB");
		ScreenPrint(scrpt2, viewProjectionMatrix, viewportMatrix, "Segment");
	}
}
