...
void ImGuiWnd()
{
	...
	if (ImGui::BeginTabBar("ObjectsTab"))
	{
		...
		if (ImGui::BeginTabItem("Triangle")) {
			...
			for (int i = 0; i < 3; i++)
			{
				sprintf_s(triangleID, "TRIANGLE_VERTICES_%d", i);
				ImGui::PushID(triangleID);
				ImGui::DragFloat3("Normal", &triangle.vertices[i].x, 0.01f);
				ImGui::PopID();
			}
		...
		if (ImGui::BeginTabItem("Collision"))
		{
			ImGui::Spacing();
			ImGui::Text("Flags");
			ImGui::Checkbox("isDrawTrisPlane", &isDrawTrisPlane);

			//ImGui::BeginTooltip();
			//ImGui::Text(R"(Enable draw plane made for "IsCollision" function)");
			//ImGui::EndTooltip();
...
void Update()
{
	...
	if (isDrawTrisPlane)
		isCollision = IsCollision(segment, triangle, viewProjectionMatrix, viewportMatrix);
	else
		isCollision = IsCollision(segment, triangle);
}
