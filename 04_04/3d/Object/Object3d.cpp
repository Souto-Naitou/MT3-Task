...
void DrawBezier(const Vector3& _controlPoint0, const Vector3& _controlPoint1, const Vector3& _controlPoint2, const Matrix4x4& _viewProjectionMatrix, const Matrix4x4& _viewportMatrix, uint32_t _color)
{
    float time = 0.0f;
    const unsigned int numPoint = 20u;
    Vector3 pointList[numPoint + 1] = {};
    for (unsigned int i = 0; i < numPoint + 1; i++)
    {
        // lerp p0,p1
        Vector3 p0p1 = Lerp(_controlPoint0, _controlPoint1, time);
        // lerp p1,p2
        Vector3 p1p2 = Lerp(_controlPoint1, _controlPoint2, time);
        // lerp p0p1,p1p2
        pointList[i] = Lerp(p0p1, p1p2, time);

        time += 1.0f / numPoint;
        if (time > 1.0f) time = 1.0f;

        if (!i) continue; // ループの最初はスキップ
        Vector3 diff = Subtract(pointList[i], pointList[i - 1]);
        DrawLine(pointList[i - 1], diff, _viewProjectionMatrix, _viewportMatrix, _color);
    }

    // 制御点の表示
    DrawSphere({ .center = _controlPoint0, .radius = 0.01f }, _viewProjectionMatrix, _viewportMatrix, BLACK);
    DrawSphere({ .center = _controlPoint1, .radius = 0.01f }, _viewProjectionMatrix, _viewportMatrix, BLACK);
    DrawSphere({ .center = _controlPoint2, .radius = 0.01f }, _viewProjectionMatrix, _viewportMatrix, BLACK);
