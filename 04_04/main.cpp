        ...
        if (ImGui::BeginTabItem("Objects")) 
        {
        	if (ImGui::BeginTabBar("TABBER_OBJECTS"))
        	{
        		if (ImGui::BeginTabItem("Ball"))
        		{
        			...
        			ImGui::PushID("PENDULUM_TABITEM");
                    if (ImGui::Button("Start", ImVec2(100,50))) isMoving = true;
                    isReset = ImGui::Button("Reset", ImVec2(100,20));
                    
                    ImGui::Spacing();
                    ImGui::Text("Translate : %f, %f, %f", ball.position.x, ball.position.y, ball.position.z);
...
void Update()
{
    ...
    if (isMoving)
    {
        ...
        if (IsCollision(Sphere{ ball.position, ball.radius }, plane))
        {
            ...
            ball.velocity = projectToNormal * 0.5f + movingDirection;
        }
    }
